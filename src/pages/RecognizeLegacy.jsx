...
const Recognize = () => {
  ...
  const handleCloseWindow = () => {
    if (window.electronAPI) {
      window.electronAPI.closeWindow(); // Use closeWindow as per main.js
    }
  };

  const handleMinimizeWindow = () => {
    if (window.electronAPI) {
      window.electronAPI.minimizeWindow(); // Use minimizeWindow as per main.js
    }
  };
    ...
    if (window.electronAPI && window.electronAPI.onFaceStatus) {
      const handleFaceStatus = (response) => { // response is already parsed JSON from main.js
        ...
        if (response.status === "success" || response.status === "success_with_attendance_error") {
          ...
          setUserPosition(response.data?.employeePosition || "N/A"); // Ensure position is set on success
          ...
          setRecentUserName(response.message || "No known face recognized."); // Allow fail message to populate name
          setUserTimeStamp("N/A"); // Clear timestamp on failure
          setUserPosition("N/A"); // Consistent: Clear position to N/A on failure
          ...
          // These fields (recentUserName, timeMarker, userTimeStamp, userPosition) are intentionally
          // NOT cleared or modified by error messages, as per the requirement.
          ...
          // These fields (recentUserName, timeMarker, userTimeStamp, userPosition) are intentionally
          // NOT cleared or modified by info messages, as per the requirement.
          ...
          // These fields are intentionally NOT cleared or modified by unexpected messages.
